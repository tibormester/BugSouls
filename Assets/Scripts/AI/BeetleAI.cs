...
public class BeetleAI : MonoBehaviour
{
    ...
    private Vector3 leftLegsStartPos;
    private Vector3 rightLegsStartPos;
    private float legsTime;
    ...
    public void Start()
    {
        ...
        leftLegsStartPos = leftLegs.transform.localPosition;
        rightLegsStartPos = rightLegs.transform.localPosition;
    }
    ...
    public float minEngageDistance = 5f, maxEngageDistance = 8f;
    ...
    public void FixedUpdate()
    {
        ...
        **/
        ...
        float moveSpeed = charMovement.GetHorizontalVelocity().magnitude;

        if (moveSpeed > 0.1f)
        {
            legsTime += Time.fixedDeltaTime * moveSpeed;
            float x = Mathf.Sin(legsTime) * 3f;

            leftLegs.transform.localPosition = new Vector3(x - 6f, leftLegs.transform.localPosition.y, leftLegs.transform.localPosition.z);
            rightLegs.transform.localPosition = new Vector3(-x - 6f, rightLegs.transform.localPosition.y, rightLegs.transform.localPosition.z);
        }
    }
    ...
    public IEnumerator LeapAttack()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            rb.AddForce(direction.normalized * 1.5f * chargeForce, ForceMode.VelocityChange);
            yield return wait;
        }
    ...
    private void ApplyDamage(Health health)
    {
        ...
        health.GetComponent<Rigidbody>()?.AddForce(direction.normalized * chargeForce, ForceMode.Impulse);
        damaged = true;
    }
